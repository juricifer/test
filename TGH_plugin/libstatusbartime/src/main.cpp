...
: SVPBaseCtrl(0, SVP_NULL, rect, pView->GetHandle())
{
    ...
    m_bmpBkgnd.Load("/media/work/sv-g4-platform/system-app/statusbar/resource/TopBar.png");
    pView->SetGeometryRect(0, 0, 800, m_bmpBkgnd.GetHeight());
...
//    connect(this, SIGNAL(signalSetPos()), this, SLOT(slotSetPos()));
    ...
    m_pPlugin = SVP_CreatePlugin(m_nIndex, "statusbar", "libstatusbartime.so.1.0.0", (ISVPBaseCtrl *)this);//不能直接传this指针，否则SVPVoid型转换为ISVPBaseCtrl型时会直接把派生类TestDlg的入口地址转换为基类ISVPBaseCtrl的地址
}
...
{
	printf("TestDlg::MsgNotify called with uMessage = %u, iIndex = %d\n", uMessage, iIndex);
