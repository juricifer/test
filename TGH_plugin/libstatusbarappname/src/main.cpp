...
//    connect(this, SIGNAL(signalSetPos()), this, SLOT(slotSetPos()));
    ...
    m_pPlugin = SVP_CreatePlugin(m_nIndex, "statusbar", "libstatusbarappname.so.1.0.0", (ISVPBaseCtrl *)this);//不能直接传this指针，否则SVPVoid型转换为ISVPBaseCtrl型时会直接把派生类TestDlg的入口地址转换为基类ISVPBaseCtrl的地址
}
